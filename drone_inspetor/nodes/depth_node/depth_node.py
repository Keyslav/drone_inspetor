...
class DepthNode(Node):
    ...
    def __init__(self):
        ...
        # A câmera depth é frontal: alimenta apenas flags de distância e o setor frontal.
        # Demais quadrantes/abaixo permanecem False (cobertos pelo lidar_node).
        self.OBSTACLES_COOLDOWN_SECONDS = 3.0
        ...
        # Publisher de obstáculos (mesmo tipo do lidar; consumido pelo drone_node)
        self.obstacles_publisher = create_publisher_from(self, Topics.Interno.DEPTH_OBSTACLE_DETECTIONS)
        self.get_logger().info(f"Publicando no tópico: {self.obstacles_publisher.topic_name}")
        ...
        # Timer para publicar obstáculos (5 Hz, alinhado com lidar_node)
        self.obstacles_timer = self.create_timer(0.2, self.publish_obstacles)
    ...
    def process_depth_image(self, depth_image):
        ...
        try:
            ...
            # ==================== ATUALIZAÇÃO DE FLAGS DE OBSTÁCULOS ====================
            self._update_obstacle_flags(filtered_depth)
    ...
    # ==================== DETECÇÃO DE OBSTÁCULOS (mesmo formato do lidar) ====================

    def _set_obstacle_flag(self, name: str, value: bool):
        """Seta uma flag com cooldown — espelha o comportamento do lidar_node."""
        ...
        if value:
            ...
            if current_time - self._flag_set_times[name] >= self.OBSTACLES_COOLDOWN_SECONDS:
                self._obstacle_flags[name] = False

    def _reset_all_obstacle_flags(self):
        """Tenta resetar todas as flags (respeita cooldown)."""
        for name in self._obstacle_flags:
            self._set_obstacle_flag(name, False)

    def _update_obstacle_flags(self, depth_image):
        """
        Atualiza flags de obstáculo a partir da imagem de profundidade já filtrada.
        A câmera é frontal: qualquer pixel próximo está no setor da frente.
        """
        valid = depth_image > 0
        if not np.any(valid):
            return

        min_dist = float(np.min(depth_image[valid]))

        self._set_obstacle_flag('have_obstacles_8m', min_dist <= 8.0)
        self._set_obstacle_flag('have_obstacles_5m', min_dist <= 5.0)
        self._set_obstacle_flag('have_obstacles_3m', min_dist <= 3.0)
        self._set_obstacle_flag('have_obstacles_2m', min_dist <= 2.0)
        self._set_obstacle_flag('have_obstacles_1m', min_dist <= 1.0)
        self._set_obstacle_flag('have_obstacles_front_90', min_dist <= 1.0)

    def _to_obstacles_msg(self) -> ObstaclesMSG:
        """Converte flags atuais para ObstaclesMSG (campos não cobertos ficam False)."""
        msg = ObstaclesMSG()
        ...
        msg.have_obstacles_right_90 = False
        msg.have_obstacles_back_90 = False
        msg.have_obstacles_left_90 = False
        msg.have_obstacles_down_1m = False
        msg.have_obstacles_down_05m = False
        return msg
    ...
    def publish_obstacles(self):
        ...
        try:
            ...
            self._reset_all_obstacle_flags()
        except Exception as e:
